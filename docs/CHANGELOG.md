...
- [修复] 个股分析兼容 MiniMax 推理模型返回的前置 `<think>` 与唯一 JSON 代码块外说明文字，同时继续拒绝多个 JSON 候选，避免有效报告被误判为 `ambiguous_json`。
...
- [文档] 新增项目功能、技术架构与二次开发指南，汇总用户侧能力、前后端运行关系、本地环境、配置、启动验证命令及 PostgreSQL、Redis、Worker 和半自动交易演进边界。
